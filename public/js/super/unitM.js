...
function openStatusModal(unitId, unitName, currentStatus) {
    const modal = document.getElementById('statusModal');
    const modalContent = modal.querySelector('.modal-content');
    const warningBox = document.getElementById('statusDeactivateWarning');
    ...
    // Show warning box only when deactivating (currentStatus is true, newStatus is false)
    if (warningBox) {
        warningBox.style.display = !newStatus ? 'block' : 'none';
    }
