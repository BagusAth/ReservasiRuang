                        ...
                        ${data.schedule_changed_data ? `
                            <div class="mt-3 p-3 bg-white rounded-lg border border-blue-200">
                                ...
                                <div class="text-xs text-gray-600 space-y-1">
                                    <p>ðŸ¢ ${escapeHtml(data.schedule_changed_data.building || '-')}</p>
                                    <p>ðŸšª ${escapeHtml(data.schedule_changed_data.room || '-')}</p>
                                    <p>ðŸ“… ${escapeHtml(data.schedule_changed_data.date || '-')}</p>
                                    <p>â° ${escapeHtml(data.schedule_changed_data.time || '-')}</p>
                                </div>
                            </div>
                        ` : ''}
                    </div>
...
async function handleRescheduleSubmit(e) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (result.conflict) {
                const conflictMsg = `${result.message}\n\nDetail Konflik:\nâ€¢ Agenda: ${result.conflict.agenda}\nâ€¢ PIC: ${result.conflict.pic_name}\nâ€¢ Tanggal: ${result.conflict.date}\nâ€¢ Waktu: ${result.conflict.time}`;
