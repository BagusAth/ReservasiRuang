...
class UserController extends Controller{
        ...
        $bookings = Booking::with(['room.building.unit'])
            ...
            ->map(function ($b) {
                return [
                    ...
                    'user_confirmation_status' => $b->user_confirmation_status,  // Include confirmation status
    ...
    /**
     * Get booking details including schedule change information.
     */
    public function getScheduleChangeDetail(int $id): JsonResponse
    {
        $user = Auth::user();
        $booking = Booking::with(['room.building'])
            ->where('user_id', $user->id)
        ...
        if (!$booking->needsUserConfirmation()) {
            return response()->json([
                'success' => false,
                'message' => 'Reservasi ini tidak memerlukan konfirmasi'
            ], 400);
        }
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'agenda_name' => $booking->agenda_name,
                'old_details' => $booking->schedule_changed_data,
                'new_details' => [
                    'room' => $booking->room->room_name ?? '-',
                    'building' => $booking->room->building->building_name ?? '-',
                ...
                'status' => $booking->status,
                'confirmation_status' => $booking->user_confirmation_status,
            ]
        ]);
    }
    
    /**
     * User confirms (approves) the schedule change.
     */
    public function confirmScheduleChange(Request $request, int $id): JsonResponse
    {
        $user = Auth::user();
        $booking = Booking::with(['room.building.unit'])
            ->where('user_id', $user->id)
        ...
        if ($action === 'approve') {
            $booking->approveScheduleChange();
            $message = 'Anda telah menyetujui perubahan jadwal reservasi';
            
            // Notify relevant admins
            $this->notifyAdminsOfScheduleConfirmation($booking, 'approved');
        } else {
            $booking->rejectScheduleChange();
            $message = 'Anda telah menolak perubahan jadwal reservasi';
            
            // Notify relevant admins
            $this->notifyAdminsOfScheduleConfirmation($booking, 'rejected');
        }
    ...
    /**
     * Notify relevant admins about user's confirmation decision.
     */
    private function notifyAdminsOfScheduleConfirmation(Booking $booking, string $confirmationType): void
    {
        ...
        $buildingAdmin = \App\Models\User::where('building_id', $building->id)
        ...
        foreach ($admins->unique('id') as $admin) {
            if ($confirmationType === 'approved') {
                \App\Models\Notification::createScheduleChangeApprovedNotification($booking, $admin);
            } else {
                \App\Models\Notification::createScheduleChangeRejectedNotification($booking, $admin);
            }
        }
    }
}